/**
 * CrawlaQueueRunner — sequential execution of all five Crawla bucket scenarios.
 *
...
/**
 * Compute EXP and HBS mock prices for a given Crawla anchor base and bucket.
 *
 * HBS is always 65% less than EXP (midpoint of the required 60-70% range):
 *   hbs = exp × 0.35
 *
 * ONLY_CRAWLA is the exception: EXP is excluded from the mock entirely, so
 * HBS is derived from the Crawla anchor directly (Crawla × 1.10).
 *
 * CHEAPEST_L2_GROSS: EXP must be cheaper than Crawla so that package-merge
 * picks EXP as the cheapest gross price and populates
 * CHEAPEST_L2_GROSS_PARTICIPATING_PACKAGES_IDS.  Factor 0.85 → EXP is 15%
 * below Crawla anchor.  marketPrice in Enigma validation = EXP gross price.
 */
function suggestPrices(base: number, bucket: CrawlaBucket): { exp: number; hbs: number } {
  // EXP excluded — HBS priced from Crawla anchor, rule doesn't apply
  if (bucket === 'ONLY_CRAWLA') return { exp: 0, hbs: Number((base * 1.1).toFixed(2)) }

  // EXP price relative to Crawla anchor
  const expFactors: Record<CrawlaBucket, number> = {
    CRAWLA_LOWER:      1.15,  // EXP higher than Crawla → Crawla wins
    EXPEDIA_LOWER:     0.85,  // EXP lower  than Crawla → Expedia wins
    EQUAL:             1.00,  // EXP ≈ Crawla
    ONLY_EXPEDIA:      1.00,  // no Crawla anchor; use fixed base of 500 below
    ONLY_CRAWLA:       1.00,  // handled above
    CHEAPEST_L2_GROSS: 0.85,  // EXP < Crawla → EXP is cheapest gross → L2 fires
  }
  ...
  // HBS always 65% less than EXP (requirement: 60-70% less)
...
function buildRequest(
  ...
  //   - Enigma's verifyRoomBasisIsROorBBAndReturnRoomBasisAsList() requires RO.
  //   - EXP room_name will differ from HBS (backend appends " L2") so they land
  //     in different similar-package groups; L2 then fires independently when
  //     EXP gross < Crawla anchor and the API key has L2 enabled.
  const rawRoomBasis = offer?.room_basis ?? offer?.meal ?? 'RO'
  const effectiveRoomBasis = bucket === 'CHEAPEST_L2_GROSS' ? 'RO' : rawRoomBasis

  // CHEAPEST_L2_GROSS: use offer refundability as-is (consistent for both
  // HBS and EXP — the backend propagates the same value to both mutations).
...
/** Poll a scenario until it reaches one of the target statuses.
 *
 *  When notFoundMeansSuccess=true, a 404 / "not found" response is treated as
 *  success — the backend deletes the row on teardown rather than setting
 *  status=TORN_DOWN, so a 404 IS the confirmation that cleanup completed.
 */
async function pollUntilStatus(
  ...
  for (let i = 0; i < maxAttempts; i++) {
    try {
      ...
      // Backend deletes the row on teardown — treat 404 as cleanup confirmed
  ...
  return (
    <div className={`qr-item qr-item--${item.status}`}>
      <div className="qr-item-header">
        ...
        {/* Search SID — populated once the run step completes */}
      ...
      {expanded && (
        <div className="qr-item-body">
          {/* Search SID — top of details, highlighted */}
          {item.runResult?.search_s_id && (
...
export function CrawlaQueueRunner() {
  ...
  // Patch a single bucket item by key
  ...
  const runAll = async () => {
    ...
    // ── Step 0: fetch anchors once for all buckets ──────────────────────────
    // Use the first bucket's phase slot for anchor-fetch feedback
    ...
    try {
      ...
      // other buckets will fall back to default prices (500/600).
    }

    patchItem('CRAWLA_LOWER', { status: 'pending', phase: undefined })

    // ── Sequential bucket execution ─────────────────────────────────────────
    for (const { bucket } of BUCKET_ORDER) {
      ...
      try {
        ...
        // called with dates 2 weeks in the future. Other buckets use the
        // user-supplied dates unchanged.
        ...
        // 3. Run Crawla scenario (reuses existing runCrawlaScenario)
        ...
        // 5. Poll until TORN_DOWN (cleanup verification).
        // notFoundMeansSuccess=true: backend deletes the row on teardown,
        // so a 404 "Scenario not found" IS the confirmation of cleanup.
        //
        // Teardown runs as a background task; if the backend job is slow or
        // fails silently (e.g. backoffice delete errors), the row stays in READY
        // and we'd timeout forever.  We treat timeout as a soft-success:
        // the teardown was initiated and this scenario's namespace is unique,
        // so there is no contamination risk for the next bucket.
        try {
          ...
          // Timeout → teardown was triggered; mark completed with a warning note.
          // Any other error (network, auth) → still proceed but surface it.
          patchItem(bucket, {
            status: 'completed',
            phase: undefined,
            error: isTimeout
